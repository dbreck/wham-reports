...
$logo_img = wham_pdf_logo_img( 'display:block;width:168px;height:auto;', 'brand-logo-img' );

$show_page_one      = wham_tier_has( $tier, 'maintenance' ) || wham_tier_has( $tier, 'gsc_aggregate' );
$show_search_tables = ! $gsc_error && ( wham_tier_has( $tier, 'gsc_top_queries' ) || wham_tier_has( $tier, 'gsc_top_pages' ) );
$show_traffic_page  = wham_tier_has( $tier, 'ga4_core' );

$render_page_header = function() use ( $client, $period_label, $tier, $logo_img ) {
	?>
	<div class="header">
		<table class="header-table">
			<tr>
				<td style="width: 50%;">
					<?php if ( $logo_img ) : ?>
						<div class="brand-logo"><?php echo $logo_img; ?></div>
					<?php else : ?>
				...
				<td class="header-right">
					...
					<div class="header-meta"><?php echo esc_html( $period_label ); ?> &middot; <?php echo esc_html( ucfirst( $tier ) ); ?> Plan</div>
				</td>
			</tr>
		</table>
	</div>
...
$render_page_footer = function() use ( $period_label ) {
	?>
	<div class="footer">
					...
					<tr>
						...
						<td class="metric-cell">
							...
							<?php if ( $prev_position ) : ?>
								...
								?>
								<div class="metric-change"><span style="font-size:8pt;color:<?php echo esc_attr( $pos_color ); ?>;font-weight:bold;"><?php echo esc_html( $pos_prefix . abs( round( $pos_change, 1 ) ) ); ?></span></div>
							<?php endif; ?>
						</td>
					</tr>
				</table>
...
<?php if ( $show_search_tables ) : ?>
<div class="page">
	<?php $render_page_header(); ?>

	<div class="content">
		<?php if ( wham_tier_has( $tier, 'gsc_top_queries' ) ) : ?>
		...
		<?php if ( wham_tier_has( $tier, 'gsc_top_pages' ) ) : ?>
				...
				<tr>
					...
					<td class="metric-cell">
						...
						<?php if ( $ga4_prev_bounce ) : ?>
							...
							?>
							<div class="metric-change"><span style="font-size:8pt;color:<?php echo esc_attr( $bounce_color ); ?>;font-weight:bold;"><?php echo esc_html( $bounce_prefix . abs( $bounce_change ) . '%' ); ?></span></div>
						<?php endif; ?>
					</td>
				</tr>
			</table>
